...
execute as @e[tag=enemy,nbt=!{HurtTime:10s},dx=0,limit=3] positioned ~-.99 ~-.99 ~-.99 if entity @s[dx=0] run function kcs:main/skill/id/sword/normal/111/damage with storage kcs:function Battle
...
execute unless score @s KCS.Skill.Range matches 31.. positioned ^ ^ ^.25 run function kcs:main/skill/id/sword/normal/111/line