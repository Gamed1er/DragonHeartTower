...
execute as @e[tag=enemy,nbt=!{HurtTime:10s},limit=3,distance=..0.9] positioned ~ ~ ~ if entity @s[] run function kcs:main/skill/id/sword/normal/211/damage with storage kcs:function Battle



execute if score @s KCS.Skill.Range matches 10.. run particle sweep_attack ~ ~ ~ 0.01 0.01 0.01 0.05 1
execute unless score @s KCS.Skill.Range matches 15.. positioned ^ ^ ^.25 run function kcs:main/skill/id/sword/normal/211/line