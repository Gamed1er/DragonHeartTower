# 排列組合釋放魔法
>Ver`0.6`
Update`250226`

## 內容
### 施法
```
	1. F魔杖至副手進入施法狀態
	2. 丟出對應屬性輸入法術ID
	3. 確認法術配方（ID)後再次F進行釋放法術
	4. 如F時當時並無紀錄ID則退出施法狀態
```
> [!NOTE]
> 投入超過上限則重置ID
    
### 自定義數值

|名稱	|命名	|描述		|
|---|---|---|
|魔量	|`OMC.Player.Mana.Points`	|魔量預設為1000實際為100.0方便計算小數	|
|消耗減免	|`OMC.Player.Mana.Points.Cost.Reduction`	|等比例減少魔量消耗，至多不超過`70%`，最低不少於`1.0/次`	|
|回魔	|`OMC.Player.Mana.Points.Regen`	|回魔預設為20義為每秒恢復20（2.0）魔力	|
|回魔增幅	|`OMC.Player.Mana.Points.Regen.Boost`	|基於基礎額外增加恢復量	|
|魔力強度	|`OMC.Player.Mana.Strength`	|魔力強度基礎預設為40實際4.0，可修改，但每次Reload後會重設	|
|施法等級	|`OMC.Skill.ID.Limit`	|施法等級預設為1，意指最多只能釋放1位元法術，可修改來釋放更高等的法術，但每次reload會重置	|

	
### 普通攻擊

1. 未處於施法狀態
2. 手持魔杖點擊`右鍵`釋放小魔球，消耗`3.0`魔量

### 魔杖的定義
* **舊版**
	- ~~物品Tags帶有["OMC.trigger"]皆為魔杖~~
	- ~~example：`/give @s stick{Tags:["OMC.trigger"]}`~~
* **新版**
	- 物品帶有下列`components`
```javascript
{components:{"minecraft:custom_data":{OMC:{trigger:true}}}}
```
#### OMC標籤
* OMC資料
```javascript
{OMC:{trigger:true,skill_id_limit:(int),wand_type:"",mana_strength:(float)}}
```
* 類別

|名稱	|類型	|
|---|---|
|skill_id_limit	|`int`	|
|wand_type	|`null`	|
|mana_strength	|`float`	|

## 資源包（預留欄位）
### 物品

|名稱		|物品	|資料	|
|---|---|---|
|無屬性元素	|`minecraft:ender_eye`	|`null`	|
|火屬性元素	|`minecraft:ender_eye`	|`null`	|
|水屬性元素	|`minecraft:ender_eye`	|`null`	|
|木屬性元素	|`minecraft:ender_eye`	|`null`	|
|土屬性元素	|`minecraft:ender_eye`	|`null`	|
|風屬性元素	|`minecraft:ender_eye`	|`null`	|
|藥引。基礎	|`null`	|`null`	|
|藥引。飛濺	|`null`	|`null`	|
|藥引。滯留	|`null`	|`null`	|
|主藥。力量	|`null`	|`null`	|
|主藥。速度	|`null`	|`null`	|
|主藥。挖掘加速	|`null`	|`null`	|
|主藥。抗性	|`null`	|`null`	|
|輔藥。紅石	|`null`	|`null`	|
|輔藥。螢光石	|`null`	|`null`	|
|輔藥。蜂蜜	|`null`	|`null`	|
|輔藥。紅石塊	|`null`	|`null`	|
|輔藥。螢石塊	|`null`	|`null`	|

### 裝備
`null`

### 武器

|名稱		|物品		|資料	|
|---|---|---|
|帝國製魔法引導杖	|`minecraft:carrot_on_a_stick`	|`null`	|
|戰鬥魔導長杖	|`minecraft:carrot_on_a_stick`	|`null`	|
|支援釀造輔杖	|`minecraft:carrot_on_a_stick`	|`null`	|
|保護醫師大型號針管	|`minecraft:carrot_on_a_stick`	|`null`	|
|通用戰鬥魔杖	|`minecraft:carrot_on_a_stick`	|`null`	|
|通用支援煉金棒	|`minecraft:carrot_on_a_stick`	|`null`	|
|通用保護銳利手術刀	|`minecraft:carrot_on_a_stick`	|`null`	|
|賢者之石<sub>_(鍊金)_</sub>	|`null`	|`null`	|
|禁忌魔法錄<sub>_(魔導)_</sub>	|`null`	|`null`	|
|腎上腺素<sub>_(醫師)_</sub>	|`null`	|`null`	|

## 配方表

### 1位元

|配方	|名稱	|描述	|數值	|
|---|---|---|---|
|`null`	|小魔彈	|向前方發射魔法彈，並對第一個擊中的目標造成傷害		|DMG`MS*0.35`	<br/>MP`3.0`	|
|無	|附魔	|釋放後短時間內任何攻擊附帶額外魔法傷害		|DMG`MS*0.5`	<br/>MP`15.0`+(`5.0`/hit)	|
|火	|點燃	|使範圍`5`內怪物燃燒5秒		|DMG`Flame`	<br/>MP`6.5`	|
|水	|打滑	|使範圍`8`內怪物`緩速I` `15`秒		|DMG`0`	<br/>MP`6.5`	|
|木	|枝枒	|對自身釋放`立即治療I`		|DMG`0`	<br/>MP`14.0`	|
|土	|包覆	|自身獲得`吸收I` `15`秒		|DMG`0`	<br/>MP`6.5`	|
|風	|乘風	|自身獲得`速度I` `10`秒	|DMG`0`	<br/>MP`5.0`	|

### 2位元

|配方	|名稱	|描述	|數值	|
|---|---|---|---|
|無無	|大魔彈	|著彈點爆炸，對半徑為`4`範圍內的怪物造成魔法傷害		|DMG`MS*0.8`	<br/>MP`8.0`	|
|火火	|炎爆彈	|每`8`點魔力強度增加`1`等爆炸強度，對範圍內所有生物造成爆炸傷害		|DMG`MS/8*n`	<br/>MP`12.0`	|
|火無	|小火球	|射出火球，對命中的敵人造成基礎`5`點傷害，如果魔力強度`100%`大於基礎傷害則覆蓋成`100%`魔力強度的燃燒傷害，並賦予燃燒`5`秒，對方塊會點火。		|DMG`5><MS*1`+`Flame`	<br/>MP`10.0`	|
|火木	|抱薪救火	|讓半徑`7`格內已燃燒的怪物燒得更旺盛，根據怪物自身剩餘燃燒時間乘以`200%+12.5*魔力強度`%，增加燃燒時間		|DMG`Flame`	<br/>MP`10.0`	|
|火風	|擴散	|將半徑`8`格內以燃燒的怪物，擴散其燃燒於其`4`格內的怪物，時間1:1		|DMG`0`	<br/>MP`12.0`	|
|水火	|沸騰點	|對自身為圓心半徑為`6`範圍內的怪物造成灼燒傷害，並且賦予`緩速II` `5`秒		|DMG`MS*0.8`	<br/>MP`11.0`	|

### 3位元

|配方	|名稱	|描述	|數值	|
|---|---|---|---|
|無無無	|奇點	|對半徑`12`格內怪物造成魔法傷害。敏光性癲癇患者請不要釋放	|DMG`MS*1.8`	<br/>MP`20.0`	|
|水火無	|蒸氣爆破	|對半徑`10`格內的怪物造成爆炸傷害，並且定身`1`秒	|DMG`MS*1.6`	<br/>MP`16.0`	|
|火土風	|易燃粉塵	|對半徑5格內燃燒的怪物造成等同於`(0.125%*魔力強度)%`的燃燒時間剩餘刻度`{Fire}`的爆炸傷害	|DMG`MS*0.00125`*`{Fire}`	<br/>MP`36.0`	|


## 更新

### 0.6 <sub>2025/02/26</sub>
* 版本提升至1.21.4

### 0.5.1 <sub>2024/07/28</sub>

	* 新增
		 - 對魔杖的魔法等級值獲取
			 - 魔杖NBT帶有{OMC:{ManaIDLimit:(賦值)}}可間接提升玩家可釋放魔法等級
	* 修改
		 - 對魔杖的魔力強度獲取
			 - `NBT：{OMCManaStrength:(賦值)} >> {OMC:{ManaStrength:(賦值}}`

### 0.5

    新增
        記分板：消耗減免(OMC.Player.Mana.Points.Cost.Reduction)、魔能恢復增幅(OMC.Player.Mana.Points.Regen.Boost)
    調整
        魔能恢復效率：
            單位由每刻恢復提升至每秒恢復(秒刻度)
                恢復值：1(per tick)>>20(per sec)
        消耗減免：
            顧名思義，至多70%減免，但不低於1.0每次消耗
        基礎魔法等級(OMC.Skill.ID.Limit)：
            2>>1

### 0.4.1

    調整：Q丟棄、小魔彈觸發
        現在只在主手手持魔杖時觸發小魔彈
        其他狀態皆可正常丟棄魔杖

### 0.4

    調整：施法狀態，ID紀錄
        施法狀態：現在紀錄ID時，F不再會同時施法並退出施法狀態，而是當下並無紀錄ID時，F退出施法狀態
        ID紀錄：現在不再使用丟出物品偵測(Q)，而是右鍵，須裝材質包(等連恩)

### 0.3.3

    修改：抱薪救火
        抱薪救火：調整倍率
            -原：魔力強度*100%
            -現：200%+12.5%魔力強度
            原版倍率疊層過快，造成易燃粉塵傷害嚴重超標，降低疊層速度以達平衡。

### 0.3.2

    新增法術：易燃粉塵
        易燃粉塵：對半徑五格內以燃燒的怪物造成等同於(0.125%*魔力強度)的燃燒時間剩餘刻度的爆炸傷害
    優化算法：抱薪救火、魔力消耗
        魔力消耗：新增記分板(OMC.Player.Mana.Points.Cost)用以方便計算消耗
    削弱：小魔球
        小魔球：50%>>35%
    

### 0.3.1

    新增法術：小火球、沸騰點、擴散、抱薪救火
        -小火球：射出一發快速的火球，對生物造成5點傷害，根據魔力強度100%計算，如果值大於5點則造成計算值的火焰傷害，如果無則仍造成5點傷害，並燃燒5秒。消耗5.5魔力值
        -沸騰點：對半徑6圓內怪物造成80%魔力強度的灼燒傷害，並且賦予緩速II|5秒
        -擴散：將半徑8格內以燃燒的怪物，擴散其燃燒於其4格內的怪物，時間1:1
        -抱薪救火：讓半徑7格內已燃燒的怪物燒得更旺盛，根據怪物自身剩餘燃燒時間與玩家魔力強度1:1倍率相乘增加燃燒時間
    修改配方：蒸氣爆破
        -蒸氣爆破：火水>>水火無

### 0.3

    現在計算魔力強度時或抓取玩家魔杖的值
        -值為自訂義標籤{OMCManaStrength:(賦值)}
        -可自行決定數值
        -example：/give @s stick{Tags:["OMC.trigger"],OMCManaStrength:16}
    現在會偵測兩件套戰鬥魔法裝備進行強化
        -詳細請至龍心規劃-戰鬥篇閱讀
        -目前只偵測兩件套
        -條件為裝備附帶標籤{Tags:["ToDH.Armor.Battle.Magic"]}
   
### 0.2.4

    完善1位元魔法
        -新增：乘風、包覆、枝枒、打滑、點燃
### 0.2.3

    新增蒸氣爆破
    修改大魔彈消耗
        -8.0>>5.5
    修改炎爆彈消耗
        -14.0>>12.0
### 0.2.2

    新增GUI提示釋放的魔法
    新增炎爆彈(ID:22)

### 0.2.1

    處於施法狀態下投入不等量且小於透入限制的魔法，F退出直接釋放其ID魔法

### 0.2

    新增簡易GUI顯示魔量於actionbar

    新增魔法[大魔彈](ID:11)
    投入(Q)兩個無屬性及釋放


