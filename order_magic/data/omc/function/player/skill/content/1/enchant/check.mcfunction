...
execute at @s as @e[nbt={HurtTime:10s}] at @s on attacker if score @s OMC.Player.ID = @p[tag=OMC.attacker.detect] OMC.Player.ID as @e[sort=nearest,limit=1] run function omc:player/skill/content/1/enchant/damage with storage omc:function Battle


tag @s remove OMC.attacker.detect
scoreboard players operation @s OMC.Player.Mana.Points -= @s OMC.Player.Mana.Points.Cost 
advancement revoke @s only omc:player/skill/id/enchant_time