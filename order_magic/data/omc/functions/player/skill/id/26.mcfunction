...
execute at @s as @s if score @s OMC.Player.Mana.Points matches 120.. as @e[tag=enemy,distance=..8] as @s unless data entity @s {Fire:-1s} as @s at @s as @e[distance=..4,nbt={Fire:-1s}] store result entity @s Fire int 1 run data get entity @e[tag=enemy,limit=1,sort=nearest,nbt=!{Fire:-1s}] Fire
