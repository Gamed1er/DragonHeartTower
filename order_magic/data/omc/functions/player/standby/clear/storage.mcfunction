...
$execute at @s run data modify block ~ ~ ~ Items set from storage omc:function Players.PC$(ID).hotbar
...
execute at @s as @e[type=marker,limit=1,sort=nearest,tag=OMC.marker.storage.return] at @s run setblock ~ ~ ~ air
