...
$execute at @s as @e[type=marker,limit=1,sort=nearest,tag=OMC.marker.storage.return] at @s run item replace entity @p[tag=OMC.Players.standby] hotbar.$(Slot) from block ~ ~ ~ container.$(Slot)
