...
execute if score @s dragon_soul >= @s keep_inventory_cost run tag @s add deal

title @s times 0t 10t 5t
execute unless score @s dragon_soul >= @s keep_inventory_cost run title @s subtitle ["",{"text":"[","bold":true,"italic":false,"color":"white"},{"translate":"ToDH.dragon_pavilion.soul_mark.dragon_soul_not_enough","bold":true,"italic":false,"underlined":true,"color":"red"},{"text":"]","bold":true,"italic":false,"color":"white"}]
title @s title [""]
...
scoreboard players add @s[tag=deal] keep_inventory_hotbar 1
scoreboard players operation @s[tag=deal] dragon_soul -= @s keep_inventory_cost
